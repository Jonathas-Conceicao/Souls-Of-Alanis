...
onready var ray_right2      = get_node( "Right2")
onready var ray_left2       = get_node( "Left2" )
...
func newData():
	...
	data.attributes.vitality += 2
	data.attributes.strength += 2
	return data
...
func _on_takeDamage(agressor, attack):
	if not current_state == state["Block"]:
		...
		print("Guardian recieved ", damage, " from: ", agressor.get_name())
		...
		var damageDisplay = DamageShower.instance()
		damageDisplay.init(self, $DamageSpot.get_position(), Vector2(1.5, 1.5), 0)
		_state_change("Walk")
	return
...
func _on_Timer2_timeout():
	if current_state == state["Block"]:
		if ray_right2.is_colliding() && direction == RIGHT:
			thrust = 1
			_state_change("Attack2")
		if ray_left2.is_colliding() && direction == LEFT:
			thrust = -1
			_state_change("Attack2")
	pass # replace with function body
